...
crime_anal_police.to_csv(ctx+'/crime_in_Seoul_include_gu_name.csv',sep=',', encoding='utf-8')
...
crime_anal_police = pd.read_csv(ctx+'/crime_in_Seoul_include_gu_name.csv', thousands=',', sep =',',  encoding='utf-8')
crime_anal_police.head()
...
crime_anal_raw = pd.read_csv(ctx+'/crime_in_Seoul_include_gu_name.csv',sep=',', encoding='utf-8')

crime_anal = pd.pivot_table(crime_anal_raw,
                            index='구별', aggfunc= np.sum)

""" 
aggfunc 은 numpy의 평균값을 리턴하는 함수
"""
crime_anal.head() 
...
평균0, 분산 1이 되도록 만드는 과정이다.
"""

x_scaled = min_max_scalar.fit_transform(x.astype(float))
# min_max_scalar(x)는 최대/최소값이 각각 1, 0 이 되도록 스케일링 
...
# Crime과 CCTV Merge 
# *******
data_result = pd.read_csv(ctx+'data_result.csv', encoding='utf-8', index_col ='구별')
data_result.head()  #최상단의 5개의 로우만 출력 
...
plt.show()
# 강도와 폭력, 살인과 폭력, 강도와 살인은 모두 양의 상관관계를 갖는다.
...
#  전체적인 상관계수는 CCTV와 살인의 관계가 낮을지 몰라도(방지효과) 
#  CCTV가 없을 때 살인이 많이 일어나는 구간이 있습니다. 
# 즉 CCTV 갯수를 기준으로 좌측면에 살인과 강도의 높은 수를 갖는 데이터가 보입니다. 
...
plt.show() 
# 그러나, 검거율과 CCTV의 관계가 양의 상관관계가 아닙니다. 
# 오히려 음의 상관관계도 보입니다. 
...
target_col = col2 
police_norm_sort = police_norm_sort.values(by='검거', ascending=False)
...
# 발생건수의 합으로 정렬해서 heatmap으로 관찰 
target_col =['강간','강도','살인','절도','폭력',"범죄"] #범죄 추가
...
map.save('C:/Users/ezen/WEEKEND_TENSORFLOW/CCTV/html/police_position.html')
...
for i in arr.index:
      folium.CircleMarker([arr['lat'][i],
                        ...
                        color ='#31868cc',
                        fill_color='#31868cc').add_to(map)
 map.save('C:/Users/ezen/WEEKEND_TENSORFLOW/CCTV/html/result.html')